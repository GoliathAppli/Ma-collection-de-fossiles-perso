...
export default function FossilDetailForm({ fossil, onSave, onDelete, onCancel }: FossilDetailFormProps) {
  ...
  const handleImageChange = (field: 'image' | 'leFossileImage', updated: ImageSettings) => {
    if (field === 'image') {
      setEdited(prev => ({ ...prev, image: updated, thumbnailImage: { ...updated } }));
    } else {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-slate-900/90 border border-yellow-700/40 rounded-2xl p-6 shadow-2xl space-y-6 text-slate-100 max-w-4xl mx-auto relative">
      ...
      {/* ZONE D'AJOUT D'IMAGE PRINCIPALE */}
      <div className="space-y-6 border-t border-slate-800/40 pt-4">
        <ImageAdjuster
          label="Photo principale du fossile (affichée en vignette dans la galerie et dans la fiche - zone transparente PNG idéale)"
      ...
      {/* RUBRIQUE DESCRIPTION SCIENTIFIQUE DE L'ESPÈCE (INCLUT L'ALIMENTATION EN SOUS-SECTION) */}
      <div className="border border-slate-850 p-4 rounded-xl space-y-4 bg-slate-950/20">
        <div className="flex items-center justify-center gap-2 border-b border-slate-800 pb-2 text-center">
          <BookOpen className="w-4 h-4 text-yellow-500" />
          <h4 className="text-sm font-semibold tracking-wide uppercase font-serif">Rubrique: Description Scientifique de l'Espèce</h4>
        </div>

        {/* 1. TEXTE DE DESCRIPTION SCIENTIFIQUE (AU-DESSUS DE LA TAILLE) */}
        <div className="space-y-1">
          <label className="block text-xs text-yellow-400 font-mono font-medium">Texte de Description Anatomique & Morphologique</label>
          <textarea
            rows={3}
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            <span className="text-xs text-slate-400 font-medium">Ligne de Photos Complémentaires de Description (Max 6)</span>
          ...
          <div className="flex flex-wrap gap-2 items-start py-2">
            {edited.descImages.map((imgUrl, i) => (
              <div key={i} className="flex-none w-56 bg-slate-900/80 p-2 border border-slate-800 rounded-lg space-y-1.5 relative">
                <div className="flex justify-between items-center">
                  <span className="text-[10px] text-slate-500 font-mono">Photo complémentaire #{i + 1}</span>
                  <button
                    type="button"
        ...
        {/* ECHELLE DES TEMPS GEOLOGIQUE POUR DETERMINER LA PERIODE DE VIE DE L'ESPECE */}
        <div className="bg-slate-950 border border-slate-850 rounded-xl p-3 mt-4 space-y-3">
          ...
          <div className="text-[10px] text-slate-400 italic">
            Faites défiler l'échelle horizontale ci-dessous et cliquez sur les périodes pour définir la période géologique de l'espèce (début et fin).
          </div>
        ...
        {/* SOUS-SECTION ALIMENTATION FAISANT PARTIE DE LA DESCRIPTION DE L'ESPÈCE */}
        <div className="border border-slate-850/90 p-4 rounded-xl space-y-3 bg-slate-950/40 mt-4">
          <div className="flex items-center justify-between border-b border-slate-800/80 pb-2">
            <div className="flex items-center gap-2">
              <Utensils className="w-4 h-4 text-yellow-500" />
              <h5 className="text-xs font-semibold tracking-wide uppercase font-serif text-white">
                Sous-section : Alimentation de l'Espèce
              </h5>
            </div>
          </div>

          <div className="space-y-1">
            <label className="block text-xs text-slate-400">Texte d'Alimentation (Optionnel)</label>
            <textarea
              rows={2}
              className="w-full bg-slate-900 border border-slate-700 text-slate-100 p-2.5 text-xs rounded focus:outline-none focus:border-yellow-600/50"
              placeholder="Détails complémentaires sur le régime alimentaire..."
              value={edited.dietText}
              onChange={(e) => handleFieldChange('dietText', e.target.value)}
            />
          </div>

          {/* SÉLECTEUR D'ICÔNES D'ALIMENTATION À COCHER */}
          <div className="pt-1">
            <DietSelector
              selectedDiets={edited.dietTypes || []}
              onChange={(updated) => setEdited(prev => ({ ...prev, dietTypes: updated }))}
            />
          </div>
        </div>
      </div>
      ...
      <div className="border border-slate-850 p-4 rounded-xl space-y-4 bg-slate-950/20">
        ...
        {/* CARACTÉRISTIQUES DU FOSSILE : DIMENSIONS & DATATION */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {/* LIGNE DIMENSIONS (TAILLE DU FOSSILE) */}
          <div className="space-y-1 bg-slate-900/50 p-3 rounded-lg border border-slate-850">
            <label className="text-xs text-yellow-400 font-mono font-medium flex items-center gap-1.5">
              <Ruler className="w-3.5 h-3.5 text-yellow-500 shrink-0" />
              <span>Dimensions du fossile (taille)</span>
            </label>
            <input
              type="text"
              className="w-full bg-slate-900 border border-slate-700 text-slate-100 px-3 py-2 text-xs rounded focus:outline-none focus:border-yellow-600/50 font-mono placeholder:text-slate-500"
              placeholder="Ex : 12,5 cm × 8 cm ou Longueur : 15 cm, Épaisseur : 3 cm..."
              value={edited.dimensions || ''}
              onChange={(e) => handleFieldChange('dimensions', e.target.value)}
            />
            <p className="text-[10.5px] text-slate-400 font-sans">
              Indiquez la taille, envergure ou mesures du spécimen.
            </p>
          </div>

          {/* LIGNE DATATION DU FOSSILE (AJOUTÉE À LA PLACE DE LA PÉRIODE DE VIE DANS CETTE RUBRIQUE) */}
          <div className="space-y-1 bg-slate-900/50 p-3 rounded-lg border border-slate-850">
            <label className="text-xs text-yellow-400 font-mono font-medium flex items-center gap-1.5">
              <Calendar className="w-3.5 h-3.5 text-yellow-500 shrink-0" />
              <span>Datation du fossile</span>
            </label>
            <input
              type="text"
              className="w-full bg-slate-900 border border-slate-700 text-slate-100 px-3 py-2 text-xs rounded focus:outline-none focus:border-yellow-600/50 font-mono placeholder:text-slate-500"
              placeholder="Ex : Dévonien moyen (~390 Ma) ou Crétacé supérieur (~70 Ma)..."
              value={edited.periodeDatation || ''}
              onChange={(e) => handleFieldChange('periodeDatation', e.target.value)}
            />
            <p className="text-[10.5px] text-slate-400 font-sans">
              Indiquez l'âge ou la datation géologique précise de ce fossile.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 items-start">
          {/* 1. PHOTO DU FOSSILE */}
          <div className="space-y-1">
            <label className="block text-xs text-slate-400 font-mono">1. Photo du fossile</label>
            <ImageAdjuster
          ...
          {/* 2. DESCRIPTION TECHNIQUE (ENTRE LA PHOTO ET LA CARTE DU MONDE) */}
          <div className="space-y-1">
            <label className="block text-xs text-slate-400 font-mono">2. Description du fossile</label>
            <textarea
              rows={8}
              className="w-full bg-slate-900 border border-slate-700 text-slate-100 p-2.5 text-xs rounded focus:outline-none focus:border-yellow-600/50"
          ...
          {/* 3. PROVENANCE GÉOGRAPHIQUE & CARTE DU MONDE */}
          <div className="space-y-3 md:col-span-2 lg:col-span-1">
            <div className="flex items-center justify-between">
              <label className="block text-xs text-yellow-400 font-mono font-medium">
                3. Provenance géographique & Carte
              </label>
              <span className="text-[10px] text-slate-500 font-mono italic">
                Champs séparés pour la fiche
              </span>
            </div>

            {/* 3 CHAMPS DISTINCTS : VILLE, PAYS, FORMATION GÉOLOGIQUE */}
            <div className="bg-slate-900/80 border border-slate-800 p-3.5 rounded-xl space-y-2.5">
              <div className="space-y-1">
                ...
                <input
                  type="text"
                  className="w-full bg-slate-950 border border-slate-700 text-slate-100 px-3 py-1.5 text-xs rounded focus:outline-none focus:border-yellow-600/50 font-mono placeholder:text-slate-600"
                  placeholder="Ex : Alnif, Millau, Solnhofen, Erfoud, Kemmerer..."
                  value={edited.provenanceVille || ''}
                  onChange={(e) => {
                    const val = e.target.value;
                    setEdited(prev => {
                      const updated = { ...prev, provenanceVille: val };
                      const combined = [updated.provenanceFormation, val, updated.provenancePays].filter(Boolean).join(' - ');
                      return { ...updated, provenanceName: combined || updated.provenanceName };
                    });
                  }}
                />
              </div>
              ...
              <div className="space-y-1">
                ...
                <input
                  type="text"
                  className="w-full bg-slate-950 border border-slate-700 text-slate-100 px-3 py-1.5 text-xs rounded focus:outline-none focus:border-yellow-600/50 font-mono placeholder:text-slate-600"
                  placeholder="Ex : Maroc, France, Allemagne, États-Unis, Liban..."
                  value={edited.provenancePays || ''}
                  onChange={(e) => {
                    const val = e.target.value;
                    setEdited(prev => {
                      const updated = { ...prev, provenancePays: val };
                      const combined = [updated.provenanceFormation, updated.provenanceVille, val].filter(Boolean).join(' - ');
                      return { ...updated, provenanceName: combined || updated.provenanceName };
                    });
                  }}
                />
              ...
              <div className="space-y-1">
                ...
                <input
                  type="text"
                  className="w-full bg-slate-950 border border-slate-700 text-slate-100 px-3 py-1.5 text-xs rounded focus:outline-none focus:border-yellow-600/50 font-mono placeholder:text-slate-600"
                  placeholder="Ex : Lits des Kem Kem, Calcaires de Solnhofen, Green River..."
                  value={edited.provenanceFormation || ''}
                  onChange={(e) => {
                    const val = e.target.value;
                    setEdited(prev => {
                      const updated = { ...prev, provenanceFormation: val };
                      const combined = [val, updated.provenanceVille, updated.provenancePays].filter(Boolean).join(' - ');
                      return { ...updated, provenanceName: combined || updated.provenanceName };
                    });
                  }}
                />
              ...
              <p className="text-[10px] text-slate-400 font-sans italic pt-1 border-t border-slate-800/80">
                * Les champs non remplis n'apparaîtront pas sur les fiches enregistrées.
              </p>
            </div>

            {/* CARTE DU MONDE ULTRA-PRÉCISE */}
            <div className="pt-1">
              <label className="block text-[11px] font-mono text-slate-400 mb-1.5">
                Localisation cartographique précise :
              </label>
              <InteractiveMap
                coords={edited.provenanceCoords}
                locationName={[edited.provenanceFormation, edited.provenanceVille, edited.provenancePays].filter(Boolean).join(' • ') || edited.provenanceName || edited.provenanceDate || "Inconnue"}
                provenanceFormation={edited.provenanceFormation}
                provenanceVille={edited.provenanceVille}
                provenancePays={edited.provenancePays}
