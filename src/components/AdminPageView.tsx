...
export default function AdminPageView({
  ...
  const handleSaveFossil = async (saved: Fossil) => {
    const cleanFossil: Fossil = {
      ...saved,
      thumbnailImage: saved.image ? { ...saved.image } : { url: '', scale: 1, posX: 0, posY: 0 },
      saviezVousImage: { url: '', scale: 1, posX: 0, posY: 0 }
    };
    const exists = config.fossils.some(f => f.id === cleanFossil.id);
    let updatedList = exists 
      ? config.fossils.map(f => f.id === cleanFossil.id ? cleanFossil : f)
      : [...config.fossils, cleanFossil];
    updatedList = sortFossilsChronologically(updatedList);

    // Mirror to technical sheets
    const hasTechSheet = config.technicalSheets.some(s => s.id === cleanFossil.id);
    const techSheetData = {
      id: cleanFossil.id,
      fossilName: cleanFossil.title,
      fossilImage: cleanFossil.leFossileImage?.url ? cleanFossil.leFossileImage : (cleanFossil.thumbnailImage?.url ? cleanFossil.thumbnailImage : cleanFossil.image),
      provenanceDate: cleanFossil.provenanceDate || "",
      periodeDatation: cleanFossil.periodeDatation || "",
      dateLieuAchat: cleanFossil.dateLieuAchat || "",
      prixAchat: cleanFossil.prixAchat || "",
      certificatImage: cleanFossil.certificatImage || { url: '', scale: 1, posX: 0, posY: 0 }
    };
  ...
  const handleDuplicateFossil = async (f: Fossil) => {
    ...
    const updatedFossils = sortFossilsChronologically([...config.fossils, duplicated]);
    const techSheetData = {
      id: duplicated.id,
            ...
            <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-4">
              ...
              <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
                <button
                  ...
                  title="Imprimer des étiquettes (7×3 cm et 5,5×3 cm) regroupées sur feuille A4"
                >
                  <Tag className="w-4 h-4 text-yellow-500" /> Étiquettes (7×3 & 5,5×3 cm)
                </button>
            ...
            {filteredFossils.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredFossils.map((fossil) => {
                  const displayImg = fossil.image?.url ? fossil.image : (fossil.leFossileImage?.url ? fossil.leFossileImage : fossil.thumbnailImage);
