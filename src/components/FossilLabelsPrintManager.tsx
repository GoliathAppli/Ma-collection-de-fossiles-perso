...
export type LabelFormat = '7x3' | '5.5x3';
...
// Adapt title font size dynamically based on length, word length & format to ensure full visibility without truncation
function getTitleTypography(title: string, format: LabelFormat = '7x3'): { fontSize: string; lineHeight: string } {
  ...
  if (format === '5.5x3') {
    if (len <= 14 && maxWordLen <= 8) {
      return { fontSize: '7.2pt', lineHeight: '1.12' };
    }
    if (len <= 20 && maxWordLen <= 10) {
      return { fontSize: '6.4pt', lineHeight: '1.10' };
    }
    if (len <= 28 || maxWordLen > 11) {
      return { fontSize: '5.6pt', lineHeight: '1.08' };
    }
    if (len <= 38) {
      return { fontSize: '5.0pt', lineHeight: '1.06' };
    }
    return { fontSize: '4.5pt', lineHeight: '1.04' };
  }
...
// Adapt provenance and datation font size dynamically to fit entirely in the frame (70mm x 30mm or 55mm x 30mm)
function getDetailsTypography(
  provenance: string,
  dating: string,
  titleLen: number,
  format: LabelFormat = '7x3'
  ...
  if (format === '5.5x3') {
    if (maxFieldLen <= 22 && totalChars <= 42 && titleLen <= 18) {
      return { fontSize: '6.0pt', lineHeight: '1.12' };
    }
    if (maxFieldLen <= 30 && totalChars <= 58 && titleLen <= 24) {
      return { fontSize: '5.4pt', lineHeight: '1.10' };
    }
    if (maxFieldLen <= 38 || totalChars <= 72) {
      return { fontSize: '4.9pt', lineHeight: '1.08' };
    }
    if (maxFieldLen <= 48 || totalChars <= 90) {
      return { fontSize: '4.4pt', lineHeight: '1.06' };
    }
    return { fontSize: '4.0pt', lineHeight: '1.04' };
  }
...
// Prominent reference badge sizing adapted to format
function getReferenceBadgeStyle(ref: string, format: LabelFormat = '7x3'): { fontSize: string; padding: string } {
  ...
  if (format === '5.5x3') {
    ...
    if (len <= 5) {
      return { fontSize: '7.2pt', padding: '0.5px 2.5px' };
    }
    return { fontSize: '6.5pt', padding: '0.5px 2px' };
  }
...
// Single Printable Label (Strict 70mm x 30mm OR 55mm x 30mm)
interface SingleLabelProps {
  fossil: Fossil;
  borderStyle: 'dashed' | 'solid' | 'none';
  labelFormat: LabelFormat;
  imageFitMode?: 'whole' | 'custom';
  isPreview?: boolean;
}
...
function SingleLabelCard({
  ...
  const imageSource = fossil.image?.url
    ? fossil.image
    : fossil.thumbnailImage?.url
    ? fossil.thumbnailImage
    : fossil.leFossileImage;

  const imageUrl = imageSource?.url ? resolveImageUrl(imageSource.url) : '';
  const scale = imageSource?.scale || 1;
  const posX = imageSource?.posX || 0;
  const posY = imageSource?.posY || 0;
  ...
  const titleTypo = getTitleTypography(title, labelFormat);
  const detailsTypo = getDetailsTypography(provenance, dating, title.length, labelFormat);
  const refStyle = getReferenceBadgeStyle(reference, labelFormat);
  ...
  const is55 = labelFormat === '5.5x3';
  const widthMm = is55 ? '55mm' : '70mm';
  const imgWidthMm = is55 ? '19.5mm' : '25.5mm';
  const imgHeightMm = is55 ? '26.4mm' : '26mm';
  const paddingMm = is55 ? '1.8mm' : '2mm';
  ...
  return (
    ...
    >
      {/* 1. PHOTO PRINCIPALE A GAUCHE EN TRANSPARENCE SUR FOND BLANC */}
      <div
      ...
      >
        {imageUrl ? (
          <img
            ...
            loading="eager"
            decoding="sync"
            className="transition-none max-w-full max-h-full"
            style={{
              width: 'auto',
              height: 'auto',
              maxWidth: '100%',
              maxHeight: '100%',
              objectFit: 'contain',
              transform: imageFitMode === 'custom' ? `scale(${scale}) translate(${posX}%, ${posY}%)` : 'none',
          ...
          <div className="w-full h-full flex flex-col items-center justify-center text-slate-300 bg-slate-50 border border-slate-200 rounded">
            <ImageIcon className={is55 ? "w-4 h-4 opacity-40" : "w-5 h-5 opacity-40"} />
          </div>
        )}
      </div>
      ...
      <div
        className="flex-1 flex flex-col justify-between h-full min-w-0 overflow-hidden"
        style={{
          paddingLeft: is55 ? '1.8mm' : '2mm',
          boxSizing: 'border-box',
        }}
      >
        ...
        <div className="min-w-0">
          {reference && (
            <span
              className="float-right ml-1 mb-0.5 inline-flex items-center justify-center font-mono font-black text-white bg-slate-950 border border-slate-900 rounded shadow-xs text-center leading-none select-all"
              style={{
      ...
      <style>{`
        ...
        @media print {
          *, *::before, *::after {
          ...
          .fossil-label-page {
            width: ${is55 ? (columns === 3 ? '175mm' : '120mm') : '202mm'} !important;
            box-sizing: border-box !important;
            ...
            grid-template-columns: ${
              is55
                ? (columns === 3 ? 'repeat(3, 55mm)' : 'repeat(2, 55mm)')
                : (columns === 3 ? 'repeat(3, 70mm)' : 'repeat(2, 70mm)')
            } !important;
            grid-auto-rows: 30mm !important;
            gap: ${
              is55
                ? (columns === 3 ? '2mm 3.5mm' : '2mm 6mm')
                : (columns === 3 ? '1.5mm 1mm' : '2mm 4mm')
            } !important;
          ...
          .fossil-label-item {
            width: ${is55 ? '55mm' : '70mm'} !important;
            height: 30mm !important;
            max-width: ${is55 ? '55mm' : '70mm'} !important;
            max-height: 30mm !important;
            min-width: ${is55 ? '55mm' : '70mm'} !important;
...
export default function FossilLabelsPrintManager({
  ...
  // Print settings
  const [labelFormat, setLabelFormat] = useState<LabelFormat>('7x3');
  const [imageFitMode, setImageFitMode] = useState<'whole' | 'custom'>('whole');
  ...
  return (
    <div className="space-y-6">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <h2 className="text-lg sm:text-xl font-bold text-white tracking-wide font-serif">
                  Impression d'Étiquettes Spécimens ({labelFormat === '5.5x3' ? '5,5 cm × 3 cm' : '7 cm × 3 cm'})
                </h2>
                <span className="text-[10px] uppercase font-mono px-2 py-0.5 rounded-full bg-yellow-950/60 text-yellow-400 border border-yellow-700/50 font-semibold">
                  {labelFormat === '5.5x3' ? 'Format 55 × 30 mm' : 'Format 70 × 30 mm'}
                </span>
              </div>
              <p className="text-xs text-slate-400 mt-1 max-w-3xl">
                Générez et imprimez des étiquettes cartels professionnelles pour vos vitrines et présentoirs.
                {labelFormat === '5.5x3'
                  ? ' Format compact 5,5 × 3 cm : photo détourée et texte (titre, référence, provenance, datation) auto-adaptés pour être entièrement affichés.'
                  : ' Format standard 7 × 3 cm : photo détourée à gauche, titre, provenance, datation, et référence à droite.'}
                {' '}Regroupement économique sur feuille A4 ({labelsPerPage} étiquettes par page).
              </p>
            </div>
          </div>
      ...
      <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        {/* Right: Format, Layout & Border settings */}
          ...
          <div className="flex items-center gap-1.5 text-xs text-slate-400">
            ...
            <div className="flex items-center bg-slate-950 border border-slate-800 p-0.5 rounded-lg">
              <button
                ...
                className={`px-2.5 py-1 rounded text-[11px] font-mono transition cursor-pointer flex items-center gap-1 ${
                  labelFormat === '7x3'
                    ? 'bg-yellow-600/30 text-yellow-400 font-bold border border-yellow-500/40'
                    : 'text-slate-400 hover:text-white'
                }`}
                title="Format standard 7 cm × 3 cm (70 × 30 mm)"
              >
                7 × 3 cm
              </button>
              <button
                ...
                title="Format compact 5,5 cm × 3 cm (55 × 30 mm) - Texte et image auto-adaptés"
              >
                5,5 × 3 cm
              </button>
            </div>
          ...
          <div className="flex items-center gap-1.5 text-xs text-slate-400">
            ...
            <div className="flex items-center bg-slate-950 border border-slate-800 p-0.5 rounded-lg">
              <button
                ...
                title="Image entière du spécimen sans coupure ni rognage (recommandé)"
              >
                Entière
              </button>
              <button
                ...
                title="Conserve le recadrage ou zoom personnalisé de la fiche"
              >
                Cadrée
              </button>
            </div>
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{
                        width: '210mm',
                        minHeight: '297mm',
                        padding: labelFormat === '5.5x3' ? '5mm 4mm' : '6mm 4mm',
                        boxSizing: 'border-box',
                      }}
                    >
                      <div
