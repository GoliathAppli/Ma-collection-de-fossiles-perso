...
// Sort helper: Cénozoïque (0) -> Mésozoïque (1) -> Paléozoïque (2) -> Précambrien (3)
const ERA_REVERSE_ORDER: Record<string, number> = {
  cenozoic: 0,
  mesozoic: 1,
  paleozoic: 2,
  precambrian: 3
...
function getEraReverseWeight(era?: string): number {
  if (!era) return 99;
  const norm = era.toLowerCase().trim();
  if (norm.includes('ceno') || norm.includes('céno')) return 0;
  if (norm.includes('meso') || norm.includes('méso')) return 1;
  if (norm.includes('paleo') || norm.includes('paléo')) return 2;
  if (norm.includes('prec') || norm.includes('préc')) return 3;
  return ERA_REVERSE_ORDER[norm] ?? 99;
}

function sortFossilsCenozoicToPrecambrian(fossils: Fossil[]): Fossil[] {
  if (!Array.isArray(fossils)) return [];
  return [...fossils].sort((a, b) => {
    const eraA = getEraReverseWeight(a.era);
    const eraB = getEraReverseWeight(b.era);
...
// Era metadata definitions ordered from Cénozoïque down to Précambrien
const ERAS_META = [
  {
...
function EraCarouselLine({
  ...
  return (
    <section className="bg-slate-950/60 border border-slate-850 hover:border-slate-800 rounded-2xl p-4 sm:p-6 space-y-4 transition-colors">
      {/* Era Line Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-yellow-700/30 pb-3.5">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <div className="flex items-center gap-2.5 flex-wrap">
              <h2 className="text-xl sm:text-2xl md:text-3xl font-extrabold font-serif uppercase tracking-wider bg-gradient-to-r from-yellow-100 via-amber-200 to-yellow-400 bg-clip-text text-transparent drop-shadow-[0_2px_10px_rgba(234,179,8,0.25)]">
                {title}
              </h2>
              <span className={`text-[10px] sm:text-xs font-mono px-2.5 py-0.5 rounded-full border shadow-sm ${badgeBg}`}>
                {fossils.length} {fossils.length > 1 ? 'spécimens' : 'spécimen'}
              </span>
            </div>
            <p className="text-xs sm:text-sm text-amber-300/80 font-mono italic mt-0.5 flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-yellow-500/70 inline-block shrink-0" />
              {dates}
            </p>
          </div>
        </div>
        ...
        >
          {fossils.map((fos) => (
            ...
            >
              ...
              {/* Title */}
              <h3 className="text-center font-serif text-sm font-bold uppercase text-amber-100/90 break-words [overflow-wrap:anywhere] hyphens-auto line-clamp-2 mb-1.5 leading-snug group-hover:text-yellow-300 transition-colors drop-shadow-sm">
...
export default function CompleteGalleryView({
  ...
  const filteredFossils = useMemo(() => {
    ...
    return sortFossilsCenozoicToPrecambrian(base);
  }, [fossils, searchQuery]);
  ...
  return (
    <div className="space-y-8 w-full max-w-6xl mx-auto pb-12">
      ...
      {/* GRAND TITRE MAJESTUEUX DE LA GALERIE */}
      <div className="text-center pt-1 pb-1 space-y-2.5 max-w-2xl mx-auto select-none">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between px-1 gap-2.5">
        <div className="flex items-center gap-1.5 text-xs font-mono text-slate-400">
          ...
          <span className="text-yellow-400/90 font-bold text-[11px]">Cénozoïque → Précambrien</span>
        </div>
        ...
        <div className="flex items-center gap-1 bg-slate-950/80 p-1 border border-slate-800/80 rounded-xl self-end sm:self-auto shadow-inner">
          <button
            ...
            className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[11px] font-mono transition-all cursor-pointer ${
              viewMode === 'carousels'
                ? 'bg-slate-800/90 text-yellow-400 border border-yellow-600/30 font-semibold shadow-xs'
                : 'text-slate-400 hover:text-slate-200 hover:bg-slate-900/60'
            }`}
            title="Affichage en carrousels horizontaux"
          >
            <Layers className="w-3.5 h-3.5 text-yellow-500/80" />
          ...
          >
            <List className="w-3.5 h-3.5 text-yellow-500/80" />
            <span>Liste</span>
          </button>
          ...
          >
            <Grid3X3 className="w-3.5 h-3.5 text-yellow-500/80" />
            ...
            ERAS_META.map((meta) => {
              ...
              return (
                <div key={meta.key} className="space-y-4">
                  ...
                  <div className="flex items-center justify-between gap-2.5 pb-2.5 border-b border-yellow-700/30">
                    <div className="flex items-center gap-3">
                      <div className={`p-2 rounded-xl ${meta.badgeBg} border ${meta.borderColor} shadow-xs`}>
                        <meta.Icon className={`w-5 h-5 ${meta.accentColor}`} />
                      </div>
                      <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                        <h3 className="font-serif text-base sm:text-lg md:text-xl font-extrabold uppercase tracking-wider bg-gradient-to-r from-yellow-100 via-amber-200 to-yellow-400 bg-clip-text text-transparent drop-shadow-sm">
                          {meta.title}
                        </h3>
                    ...
                    <span className="text-[10px] sm:text-xs font-mono bg-slate-900/90 border border-slate-800 text-slate-300 px-3 py-1 rounded-full shrink-0 shadow-xs">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-5">
                    {eraFossils.map((fos) => (
                      ...
                      >
                        ...
                        <h3 className="text-center font-serif text-sm font-bold uppercase text-amber-100/90 break-words line-clamp-2 mb-1 leading-snug group-hover:text-yellow-300 transition-colors drop-shadow-sm">
                          {fos.title || 'Spécimen de Fossile'}
                        </h3>
                ...
                <div key={meta.key} className="space-y-3 sm:space-y-4">
                  ...
                  <div className="flex items-center justify-between gap-2.5 pb-2.5 border-b border-yellow-700/30">
                    <div className="flex items-center gap-2.5">
                      <div className={`p-1.5 rounded-lg ${meta.badgeBg} border ${meta.borderColor} shadow-xs`}>
                        <meta.Icon className={`w-4 h-4 sm:w-4.5 sm:h-4.5 ${meta.accentColor}`} />
                      </div>
                      <div className="flex flex-wrap items-baseline gap-x-2.5 gap-y-0.5">
                        <h3 className="font-serif text-sm sm:text-base md:text-lg font-extrabold uppercase tracking-wider bg-gradient-to-r from-yellow-100 via-amber-200 to-yellow-400 bg-clip-text text-transparent drop-shadow-sm">
                          {meta.title}
                        </h3>
                        <span className="text-[10px] sm:text-xs font-mono text-amber-300/80 font-medium">
                          {meta.dates}
                        </span>
                      </div>
                    </div>
                    <span className="text-[9px] sm:text-[11px] font-mono bg-slate-900/90 border border-slate-800 text-slate-300 px-2.5 py-0.5 rounded-full shrink-0 shadow-xs">
                      {eraFossils.length} spécimen{eraFossils.length > 1 ? 's' : ''}
                    </span>
                  </div>
                    ...
                    {eraFossils.map((fos) => (
                      ...
                      >
                        ...
                        <div className="flex-1 flex flex-col justify-between">
                          <h3 className="text-center font-serif text-[11px] sm:text-xs md:text-sm font-bold uppercase text-amber-100/90 break-words line-clamp-2 mb-1 leading-tight group-hover:text-yellow-300 transition-colors drop-shadow-sm">
                            {fos.title || 'Spécimen de Fossile'}
                          </h3>
