...
export default function FossilPrintTemplate({ fossil, sheet }: FossilPrintTemplateProps) {
  ...
  const provenanceParts = [fossil.provenanceFormation, fossil.provenanceVille, fossil.provenancePays].filter(Boolean);
      ...
      <div className="w-full h-full border-[2px] border-slate-900 outline outline-1 outline-slate-800 outline-offset-1 p-[3.5mm] flex flex-col justify-between relative bg-white box-border font-serif select-none">
        ...
        <div className="grid grid-cols-2 gap-2 flex-1 min-h-0 items-stretch my-1">
              ...
              <div className={`${cardPadding} flex-1 flex flex-col justify-between gap-1.5 min-h-0`}>
                ...
                {(fossil.lifespanPeriodStart || fossil.lifespanPeriodEnd || fossil.tailleEspece || fossil.poidsEspece) && (
                  <div className="text-[8px] font-mono font-bold text-slate-800 py-0.5 border-t border-b border-slate-200 flex flex-wrap items-center gap-x-2 gap-y-0.5 shrink-0 bg-slate-50/80 px-1 rounded">
                    ...
                    {(fossil.lifespanPeriodStart || fossil.lifespanPeriodEnd) && (
                      <span className="flex items-center gap-1">
                        <span className="text-slate-500 uppercase tracking-wider text-[7px]">Période de vie :</span>
          ...
          <div className="flex flex-col gap-2 justify-between h-full min-h-0">
            ...
            <div className="border border-slate-800 flex flex-col justify-between bg-white flex-1 min-h-0 relative shadow-xs">
              ...
              <div className={`${cardPadding} flex-1 flex flex-col justify-between gap-1.5 min-h-0`}>
                {fossil.dietTypes && fossil.dietTypes.length > 0 && (
                  <div className="flex flex-wrap gap-1 pb-1 border-b border-slate-200 shrink-0">
                    {fossil.dietTypes.map((dt) => (
                      <span key={dt} className="px-1.5 py-0.5 rounded bg-slate-100 border border-slate-300 text-slate-900 text-[7.5px] font-bold font-mono">
                        • {dt}
                      </span>
                    ))}
                  </div>
                ...
                <p className={`${textSize} text-slate-900 text-justify whitespace-pre-wrap flex-1 overflow-hidden`}>
                  {fossil.dietText || 'Données sur l’alimentation et le régime trophique documentées pour cette espèce.'}
                </p>
              </div>
            </div>
            ...
            <div className="border border-slate-800 flex flex-col justify-between bg-white flex-1 min-h-0 relative shadow-xs">
              <div className="bg-slate-900 text-white px-2 py-0.5 font-sans text-[8px] font-black uppercase tracking-wider border-b border-slate-800 shrink-0 flex justify-between items-center">
                <span>🔬 Le Fossile — Spécimen de Collection</span>
                <span className="text-[7px] text-amber-300 uppercase tracking-widest">Spécimen Réel</span>
              </div>
              <div className={`${cardPadding} flex-1 flex flex-col justify-between gap-1.5 min-h-0`}>
                {(fossil.dimensions || effectivePeriod) && (
                  <div className="text-[8px] font-mono font-bold text-slate-800 pb-0.5 border-b border-slate-200 flex flex-wrap items-center gap-x-2 gap-y-0.5 shrink-0">
                    {fossil.dimensions && (
                      <span className="flex items-center gap-1">
                        <span className="text-slate-500 uppercase tracking-wider text-[7px]">Dimensions :</span>
                        <span className="text-slate-900 font-semibold">{fossil.dimensions}</span>
                      </span>
                    )}
                    {effectivePeriod && (
                      <span className="flex items-center gap-1">
                        <span className="text-slate-500 uppercase tracking-wider text-[7px]">Datation :</span>
                        <span className="text-slate-900 font-semibold">{effectivePeriod}</span>
                      </span>
                    )}
                  </div>
                )}
