...
export default function CroppedImage({ settings, alt = 'Fossil Image', className = '', onClick, style }: CroppedImageProps) {
  ...
  return (
    <>
      <div
        onClick={handleClick}
        className={`relative overflow-hidden rounded bg-transparent group cursor-pointer ${className}`}
        style={style}
      >
      ...
      {isFullscreen && typeof document !== 'undefined' && createPortal(
        <div 
          className="fixed inset-0 z-[99999] bg-black/95 flex flex-col items-center justify-center p-4 backdrop-blur-md cursor-zoom-out"
        ...
        >
          ...
          <img 
            src={resolveImageUrl(settings.url)} 
            alt={alt}  
            className="max-w-full max-h-full object-contain shadow-2xl rounded select-none cursor-default"
