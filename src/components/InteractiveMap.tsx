...
/**
 * Checks if a fossil's provenance is unknown, unspecified or non-documented
 */
export function isUnknownLocation(locationName?: string): boolean {
  ...
  if (
    normalized.includes("inconnue") ||
    normalized.includes("inconnu") ||
    normalized.includes("unknown") ||
    normalized.includes("indetermine")
  ) {
    return true;
  }
...
export function resolveCoords(
  ...
  // If location is unknown, show global world overview
  if (isUnknownLocation(locationName)) {
    return { lat: 20, lng: 0, zoom: 2, source: 'unknown' };
  }
...
export interface InteractiveMapProps {
  ...
  readOnly?: boolean;
  cardTitle?: string;
  provenanceFormation?: string;
  provenanceVille?: string;
  provenancePays?: string;
}
...
export default function InteractiveMap({ 
  ...
  // Compute whether the provenance is unknown
  const hasSpecificFields = Boolean(provenanceFormation || provenanceVille || provenancePays);
  const isUnknown = !hasSpecificFields && isUnknownLocation(locationName);
  ...
  // Helper to ensure the marker is created or updated
  const updateOrAddMarker = useCallback((lat: number, lng: number, label: string, targetMap?: L.Map) => {
    ...
    if (!markerRef.current) {
      ...
      markerRef.current.setIcon(createPinIcon(label));
      markerRef.current.setPopupContent(popupHtml);
    }
  }, [createPinIcon, readOnly]);
  ...
  useEffect(() => {
    ...
    if (!mapInstanceRef.current) {
      const map = L.map(containerRef.current, {
        center: isUnknown ? [20, 0] : [resolved.lat, resolved.lng],
        zoom: isUnknown ? 2 : resolved.zoom,
        minZoom: 1,
      ...
      // Only add marker if location is actually known
      if (!isUnknown) {
        updateOrAddMarker(resolved.lat, resolved.lng, locationName || "Lieu de découverte", map);
      }
      ...
      if (!readOnly) {
        map.on('click', (e) => {
          playDinoSound();
          const targetName = searchQueryRef.current && !isUnknownLocation(searchQueryRef.current)
            ? searchQueryRef.current
            : "Gisement personnalisé";
          updateOrAddMarker(e.latlng.lat, e.latlng.lng, targetName, map);
          if (onChangeRef.current) {
            onChangeRef.current({ lat: e.latlng.lat, lng: e.latlng.lng }, targetName);
          }
      ...
      if (isUnknown) {
        // When location is unknown, remove marker and fly out to world overview
        if (markerRef.current) {
        ...
        // Location is known: place or update marker and pan/fly smoothly
        updateOrAddMarker(resolved.lat, resolved.lng, locationName || "Lieu de découverte", map);

        const currentCenter = map.getCenter();
        const dist = Math.hypot(currentCenter.lat - resolved.lat, currentCenter.lng - resolved.lng);
        if (dist > 0.005) {
          map.flyTo([resolved.lat, resolved.lng], Math.max(6, resolved.zoom), { animate: true, duration: 1.2 });
        }
      }
    }
  ...
  }, [resolved.lat, resolved.lng, resolved.zoom, locationName, isUnknown, readOnly, updateOrAddMarker]);
  ...
  // Center view on marker (or global map if unknown)
  ...
  const executeSearch = async (query: string) => {
    ...
    if (offlineMatch) {
      ...
      playDinoSound();
      if (mapInstanceRef.current) {
        updateOrAddMarker(offlineMatch.lat, offlineMatch.lng, offlineMatch.label || query);
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        if (mapInstanceRef.current) {
          updateOrAddMarker(lat, lng, cleanName || query);
  ...
  return (
    ...
    >
      {/* Titre intégré à l'intérieur de l'encadré */}
      {cardTitle && (
        <div className="flex items-center justify-center gap-2 pb-2.5 border-b border-slate-800/80 text-slate-200 text-xs font-serif font-bold uppercase tracking-wider text-center shrink-0">
      ...
      <div className={`flex flex-col sm:flex-row sm:items-center justify-between gap-2 px-1 ${isFullscreen ? 'p-3 border-b border-slate-800' : ''}`}>
        <div className="flex items-center gap-2">
          <div className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 shadow ${
            ...
            {isUnknown ? <Compass className="w-4 h-4" /> : <MapPin className="w-4 h-4" />}
          </div>
          <div>
            {hasSpecificFields ? (
              <div className="flex flex-wrap items-center gap-1.5 py-0.5">
                {provenanceFormation && (
                  <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md bg-yellow-500/10 border border-yellow-500/30 text-yellow-300 font-serif text-xs">
                    <span className="text-[10px] font-sans text-yellow-500 uppercase tracking-wider font-semibold">Formation :</span>
                    <span>{provenanceFormation}</span>
                  </span>
                )}
                {provenanceVille && (
                  <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md bg-slate-900 border border-slate-750 text-slate-200 font-serif text-xs">
                    <span className="text-[10px] font-sans text-slate-400 uppercase tracking-wider font-semibold">Ville :</span>
                    <span>{provenanceVille}</span>
                  </span>
                )}
                {provenancePays && (
                  <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md bg-slate-900 border border-slate-750 text-slate-200 font-serif text-xs">
                    <span className="text-[10px] font-sans text-slate-400 uppercase tracking-wider font-semibold">Pays :</span>
                    <span>{provenancePays}</span>
                  </span>
                )}
              </div>
            ) : (
              <h4 className="text-xs sm:text-sm font-serif font-bold text-white tracking-wide">
                {isUnknown ? "Localisation d'origine inconnue" : (locationName || "Lieu de découverte non renseigné")}
              </h4>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          {/* GPS Coordinates chip with click-to-copy (only when known) */}
          {!isUnknown ? (
            <button
              type="button"
            ...
            <span className="flex items-center gap-1 bg-slate-900 border border-amber-900/40 text-amber-400/80 px-2 py-1 rounded text-[10px] font-mono">
              <Compass className="w-3 h-3 text-amber-400" /> Planisphère global
            </span>
          )}
      ...
      >
        ...
        {/* Grand point d'interrogation au milieu quand la localisation est inconnue */}
        {isUnknown && (
          <div 
            id="fossil-map-unknown-overlay"
            className="absolute inset-0 z-10 flex items-center justify-center pointer-events-none select-none"
          >
            <div className="relative flex items-center justify-center">
              {/* Halo lumineux ambré */}
              <div className="absolute w-32 h-32 sm:w-44 sm:h-44 rounded-full bg-amber-500/20 blur-2xl animate-pulse" />

              {/* Médaillon central contenant le grand point d'interrogation */}
              <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-full bg-slate-950/75 border-2 border-amber-500/60 backdrop-blur-md shadow-[0_0_35px_rgba(245,158,11,0.45)] flex items-center justify-center">
                <span className="text-5xl sm:text-6xl font-serif font-black text-amber-400 drop-shadow-[0_4px_16px_rgba(245,158,11,0.8)] leading-none select-none">
                  ?
                </span>
              </div>
            </div>
        ...
        <div className="absolute bottom-2.5 right-2.5 z-10 flex flex-col gap-1 shadow-2xl">
          <button
            ...
            title={isUnknown ? "Vue globale du planisphère" : "Recadrer sur le lieu de découverte"}
          >
